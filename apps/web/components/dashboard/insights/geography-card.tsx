...
export function GeographyCard({ countries, cities }: GeographyCardProps) {
  const [geoTab, setGeoTab] = useState<"countries" | "cities">("countries")

  const activeList = geoTab === "countries" ? countries : cities
      ...
      <div className="flex-1">
        {list.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-[260px] text-slate-400">
            <p className="text-xs font-semibold">No location data available</p>
          </div>
        ) : (
          <div className="space-y-4 h-[260px] overflow-y-auto pr-1">
            {sortedList.map((item, idx) => {
              const percentage = total > 0 ? (item.count / total) * 100 : 0
              const displayName = item.name ? item.name : "Unknown Location"
